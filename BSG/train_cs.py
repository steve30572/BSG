...
def train_nodeclas(model, data, args, device='cpu'):
    ...
    # if np.mean(results) >= 0.836:
    if np.mean(results) >= 0.93:
        f = open(f'./result_cs.txt', 'a')
        f.write(f'{args.alpha}, {args.beta}, {args.gamma}, {args.margin} {args.nodeclas_weight_decay}, {args.alpha}, {args.decoder_channels}, {args.hidden_channels}, {args.encoder_channels}, {args.encoder_dropout}, {args.p}, {args.lr}, {args.weight_decay}: {np.mean(results):.2%} ± {np.std(results):.2%} \n') # Cora: final acc, Citeseer: early-stop acc
        #f.write(f'{alpha}, {beta} : {estp_acc:.4f}±{estp_acc_std:.4f} \n') # Cora: final acc, Citeseer: early-stop acc
        f.close()
        # torch.save(embedding, f'{np.mean(results):.2%}_cora.pt')
        # torch.save(embedding, f'original_cora_bn.pt')
...
# parser.add_argument('--alpha', type=float, default=random.choice([0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]), help='loss weight for degree prediction. (default: 0.)')
...
parser.add_argument("--layer", nargs="?", default="gcn", help="GNN layer, (default: gcn)")
parser.add_argument("--encoder_activation", nargs="?", default="elu", help="Activation function for GNN encoder, (default: elu)")
parser.add_argument('--encoder_channels', type=int, default=random.choice([256, 512, 64, 128, 1024]), help='Channels of GNN encoder layers. (default: 128)')
parser.add_argument('--hidden_channels', type=int, default=random.choice([256, 512, 64, 128, 1024]), help='Channels of hidden representation. (default: 64)')
parser.add_argument('--decoder_channels', type=int, default=random.choice([256, 32, 64, 128, 1024]), help='Channels of decoder layers. (default: 128)')
parser.add_argument('--encoder_layers', type=int, default=2, help='Number of layers for encoder. (default: 2)')
parser.add_argument('--decoder_layers', type=int, default=2, help='Number of layers for decoders. (default: 2)')
parser.add_argument('--encoder_dropout', type=float, default=random.choice([0.4, 0.5, 0.6, 0.7, 0.8, 0.9]), help='Dropout probability of encoder. (default: 0.8)')
parser.add_argument('--decoder_dropout', type=float, default=0.2, help='Dropout probability of decoder. (default: 0.2)')
parser.add_argument('--alpha', type=float, default=random.choice([0.1, 0.01, 1e-3, 1e-4]), help='loss weight for degree prediction. (default: 0.)')
# parser.add_argument('--alpha', type=float, default=round(random.uniform(0., 0.001), 5), help='loss weight for degree prediction. (default: 0.)')

parser.add_argument('--lr', type=float, default=random.choice([0.02, 0.01, 0.001, 0.005, ]), help='Learning rate for training. (default: 0.01)')
parser.add_argument('--weight_decay', type=float, default=5e-5)#random.choice([1e-4, 5e-4, 5e-5]), help='weight_decay for link prediction training. (default: 5e-5)')
parser.add_argument('--grad_norm', type=float, default=1.0, help='grad_norm for training. (default: 1.0.)')
parser.add_argument('--batch_size', type=int, default=2**16, help='Number of batch size for link prediction training. (default: 2**16)')

parser.add_argument("--start", nargs="?", default="node", help="Which Type to sample starting nodes for random walks, (default: node)")
parser.add_argument('--p', type=float, default=random.choice([0.7]), help='Mask ratio or sample ratio for MaskEdge/MaskPath')

parser.add_argument('--bn', action='store_true', help='Whether to use batch normalization for GNN encoder. (default: False)')
parser.add_argument('--l2_normalize', action='store_true', help='Whether to use l2 normalize output embedding. (default: False)')
parser.add_argument('--nodeclas_weight_decay', type=float, default=random.choice([1e-3, 1e-4, 1e-5]), help='weight_decay for node classification training. (default: 1e-3)')
...
parser.add_argument("--device", type=int, default=0)
parser.add_argument('--full_data', action='store_true', help='Whether to use full data for pretraining. (default: False)')
parser.add_argument('--beta', type=float, default=round(random.uniform(0., 0.001), 4))
# parser.add_argument('--beta', type=float, default=random.choice([0.1, 0.01,0.001, 0.0001, 1e-4]))
parser.add_argument('--gamma', type=float, default=round(random.uniform(0., 0.001), 4))
# parser.add_argument('--gamma', type=float, default=random.choice([0.1, 0.01,0.001, 0.0001, 1e-4]))
# parser.add_argument('--beta', type=float, default=round(random.uniform(0., 0.1), 4))
# parser.add_argument('--gamma', type=float, default=round(random.uniform(0., 0.001), 4))
# parser.add_argument('--beta', type=float, default=round(random.uniform(0., 2), 4))
# parser.add_argument('--gamma', type=float, default=round(random.uniform(0., 2), 4))
# parser.add_argument('--gamma', type=float, default=round(random.uniform(100., 120), 4))
parser.add_argument('--margin', type=float, default=round(random.uniform(-0.5, 0.5), 1))
...
model = MaskGAE(encoder, edge_decoder, degree_decoder, mask, data.num_nodes).to(device)
