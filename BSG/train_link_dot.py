...
from maskgae.model_dot import MaskGAE, DegreeDecoder, EdgeDecoder, GNNEncoder, DotEdgeDecoder
...
# f = open(f'./link_physics.txt', 'a')
# f.write(f'{args.alpha}, {args.beta}, {args.gamma}, {args.margin}, {args.alpha}, {args.decoder_channels}, {args.hidden_channels}, {args.encoder_channels}, {args.encoder_dropout}, {args.p}, {args.lr}, {args.weight_decay}: {np.mean(auc_results):.2%} ± {np.std(auc_results):.2%}, {np.mean(ap_results):.2%} ± {np.std(ap_results):.2%} \n') # Cora: final acc, Citeseer: early-stop acc
