...
from torch_geometric.utils import add_self_loops, negative_sampling, degree, to_dense_adj
from torch_sparse import SparseTensor
from torch.utils.data import DataLoader
from sklearn.metrics import roc_auc_score, average_precision_score, pairwise_distances
from torch_geometric.nn import MessagePassing, DenseGCNConv, dense_diff_pool
import numpy as np
...
class Pool(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.S = nn.Parameter(torch.randn(in_dim, out_dim))
        nn.init.xavier_normal_(self.S, gain=1.414)
        # self.mha = nn.MultiheadAttention(in_feature, num_heads=8)


    def forward(self, x, mask_nodes=None, pool_size=None):
        if len(x.shape) == 3:
            m = nn.Sigmoid()
            s = m(self.S)
            b, n, f = x.shape
            ...
            x = x.reshape(n, -1)
            new_feature = torch.matmul(s.T, x)
            new_feature = new_feature.reshape(-1, b, f)
            new_feature = torch.transpose(new_feature, 0, 1)
            return new_feature, s
            ...
            if pool_size is not None:
                x = x[:pool_size]
            new_feature = torch.matmul(s.T, x) # out_dim * feature_dim
            # new_feature, _ = self.mha(new_feature, new_feature, new_feature)
            return new_feature, s
...
class MaskGAE(nn.Module):
    def __init__(
        ...
        # self.pool_size = input_dim
        # self.pool = Pool(input_dim, input_dim // 10) # Cora, Citeseer 4
        # self.pool_reverse = Pool(input_dim // 10, input_dim)
    ...
    def train_step(self, data, optimizer, alpha=0.002,
        batch_size=2 ** 16, grad_norm=1.0, beta=0.002, gamma=0.002, margin=-0.5
    ):
        self.train()
        ...
        for perm in DataLoader(
            ...
            # We need to subtract the mean from each column (feature-wise)
            # mean_data = torch.mean(z, dim=0, keepdim=True)
            # data_centered = z - mean_data

            # Calculate the covariance matrix manually using the centered data
            # Covariance = (1 / (n - 1)) * X^T X
            # n_samples = z.shape[0]
            # cov_matrix = (1 / (n_samples - 1)) * torch.matmul(data_centered.T, data_centered)

            # # Step 2: Calculate the entropy for a multivariate Gaussian
            # d = z.shape[1]  # Number of dimensions (features)

            # # Determinant of the covariance matrix
            # det_cov = torch.det(cov_matrix)

            # # Ensure determinant is positive (to avoid numerical issues)
            # if det_cov <= 0:
            #     det_cov = torch.tensor(1e-10)
            ...
            # loss += gamma * torch.mean(torch.relu(similarity_matrix_edge - torch.min(similarity_matrix_edge)))
            ...
            # with torch.no_grad():
            #     z_new = self.encoder(x, edge_index)
            #     z_temp = z_new / torch.norm(z_new, dim=1, p=2).reshape(-1, 1)
            #     D = 1 - torch.matmul(z_temp, z_temp.T)
            #     D_gen = torch.where(D > 0, torch.tensor(1), torch.tensor(0))
            #     D_tgt = torch.sum(D, dim=1, keepdim=True)/torch.sum(D_gen, dim=1, keepdim=True)
            #     D_gen = torch.where(D_tgt > 0, torch.tensor(1), torch.tensor(0))
            #     MAD = torch.sum(D_tgt)/torch.sum(D_gen)
            #     print(MAD.shape, MAD)
            with torch.no_grad():
                z_new = self.encoder(x, remaining_edges)
                # z_temp = z_new / torch.norm(z_new, dim=1, p=2).reshape(-1, 1)
                ...
                mad = np.mean(node_dist)
                print(mad, F.mse_loss(z, aggregated_message))
