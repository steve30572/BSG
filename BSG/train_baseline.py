...
def train_nodeclas(model, data, args, device='cpu'):
    ...
    # if np.mean(results) >= 72:
    f = open(f'./result_arxiv.txt', 'a')
...
parser.add_argument('--encoder_channels', type=int, default=random.choice([256, 512, 1024, 2048, 4096, 128]), help='Channels of GNN encoder layers. (default: 128)')
parser.add_argument('--hidden_channels', type=int, default=random.choice([256, 512, 1024, 2048, 64, 128]), help='Channels of hidden representation. (default: 64)')
parser.add_argument('--decoder_channels', type=int, default=random.choice([256, 512, 1024, 32, 64, 128]), help='Channels of decoder layers. (default: 128)')
...
parser.add_argument('--lr', type=float, default=random.choice([0.01, 0.001, 0.0001]), help='Learning rate for training. (default: 0.01)')
parser.add_argument('--weight_decay', type=float, default=random.choice([1e-4, 5e-4, 5e-5]), help='weight_decay for link prediction training. (default: 5e-5)')
...
parser.add_argument('--p', type=float, default=random.choice([0.6, 0.7, 0.8]), help='Mask ratio or sample ratio for MaskEdge/MaskPath')
...
parser.add_argument('--nodeclas_weight_decay', type=float, default=random.choice([0, 5e-4, 1e-4, 5e-5, 1e-5]), help='weight_decay for node classification training. (default: 1e-3)')
...
class GCN(torch.nn.Module):
    def __init__(self, input_feat, hidden_feat, output_feat):
        super(GCN, self).__init__()
        self.layer1 = torch_geometric.nn.GCNConv(input_feat, hidden_feat)
        self.layer2 = torch_geometric.nn.GCNConv(hidden_feat, output_feat)
...
class GAT(torch.nn.Module):
    def __init__(self, input_feat, hidden_feat, output_feat):
        ...
        self.layer1 = torch_geometric.nn.GATConv(input_feat, hidden_feat)
        self.layer2 = torch_geometric.nn.GATConv(hidden_feat, output_feat)
...
class MLP(torch.nn.Module):
    def __init__(self, input_feat, hidden_feat, output_feat):
        ...
        self.layer1 = nn.Linear(input_feat, hidden_feat)
        self.layer2 = nn.Linear(hidden_feat, output_feat)
    ...
    def forward(self, x, edge_index):
        ...
        x = F.log_softmax(x, dim=1)
        return x
model = MLP(train_data.x.shape[1], 128, len(torch.unique(train_data.y)))
